...
#ifdef _WIN32
		vMin.x = __min(vMin.x,v.x);
		vMax.x = __max(vMax.x,v.x);
		vMin.y = __min(vMin.y,v.y);
		vMax.y = __max(vMax.y,v.y);
		vMin.z = __min(vMin.z,v.z);
		vMax.z = __max(vMax.z,v.z);
#else
...
#ifdef _WIN32
  vMax.x = __max(vMax.x * 0.99, (double)(vMin.x + epsilon));
  vMax.y = __max(vMax.y * 0.99, (double)(vMin.y + epsilon));
  vMax.z = __max(vMax.z * 0.99, (double)(vMin.z + epsilon));
#else
  vMax.x = std::max(vMax.x * 0.99, (double)(vMin.x + epsilon));
  vMax.y = std::max(vMax.y * 0.99, (double)(vMin.y + epsilon));
  vMax.z = std::max(vMax.z * 0.99, (double)(vMin.z + epsilon));
#endif
#else
  prior_y_Max = vMax.y;
