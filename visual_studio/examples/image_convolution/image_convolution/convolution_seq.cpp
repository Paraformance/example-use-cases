...
double get_current_time()
{
  ...
  return (double) t1.QuadPart * 1000000LL / (double) frequency.QuadPart;
#else
...
ushort *read_image(const char *fileName, png_uint_32 height) {
  ...
  png_read_info(png_ptr, info_ptr); //error here
...
string_p get_image_name(int i) {
	string_p image_n = new string("..\\image_convolution\\images\\image" + to_string(i) + ".png");
...
#ifdef _WIN32
  unsigned int pattern, do_chunking, min_chunk_size;
  int i = 0;
  // if (argc<3)
  //  std::cerr << "use: " << argv[0] << " <imageSize> <nrImages> [<chunking>]\n";
  dim = 1024; // atoi(argv[1]);

  unsigned const int nr_images = 20; // atoi(argv[2]);
#else
