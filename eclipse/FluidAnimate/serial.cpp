...
//core of benchmark program (the Region-of-Interest)
  ...
  auto start = std::chrono::steady_clock::now();  // Added for ParaPhrasing

  for(int i = 0; i < framenum; ++i)
    AdvanceFrame();

  auto end = std::chrono::steady_clock::now();    // Added for ParaPhrasing
  std::cout << std::endl << "Simulation time: " << std::chrono::duration <double, std::milli> (end - start).count() << " [ms]" << std::endl;
